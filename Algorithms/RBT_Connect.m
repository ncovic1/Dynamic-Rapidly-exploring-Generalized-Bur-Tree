...
methods
    function this = RBT_Connect(eps, N_max, N_spines, d_crit, delta)
        if nargin > 0
            this.eps = eps;
            this.N_max = N_max;
    ...
    function [q_new, reached] = GenerateSpine(this, q, q_e, d_c)
        global robot;
        % Spine is generated from q towards q_e using complete bubble
        ...
        K_max = 1;  % Number of iterations for computing q*
        k = 1;
        while true            
            step = this.ComputeStep(q_new, q_e, d_c-rho, xyz_q_new, robot.N_links);            
            if step > 1
                q_new = q_e;
                reached = true;
            else          
    ...
    function [q_new, reached] = GenerateSpine2(this, q, q_e, d_c)
        global robot;
        % Spine is generated from q towards q_e using expanded bubbles
        % q_new represents new reached node
        % reached means whether q_e is reached
        % d_c is a vector containing distances from each robot link to obstacles
        ...
        [xyz_q, ~] = DirectKinematics(robot, q);
        xyz_q_new = xyz_q;
        Rho = zeros(robot.N_links, 1);   % Path length in W-space
        K_max = 1;  % Number of iterations for computing q*
        k = 1;
        while true
            step = zeros(robot.N_links, 1);
            for n = 1:robot.N_links
                step(n) = this.ComputeStep(q_new, q_e, d_c(n)-Rho(n), xyz_q_new, n);   % 'd_c-rho' is the remaining path length in W-space
            end
            step = min(step);
            ...
            rho_i1 = 0;
            for i = 2:robot.N_links+1
                 rho_i = norm(xyz_q_new(:,i)-xyz_q(:,i));
                 Rho(i-1) = max([rho_i1, rho_i]);
                 rho_i1 = rho_i;
            end
            k = k + 1;
        end
    ...
    function step = ComputeStep(~, q, q_e, fi, xyz, n)
        global robot;
        
        if robot.dim == 2   % assumes that N_links = N_DOF
            d = 0;
            for ii = 1:n
                r = [robot.DH_table(ii,2), zeros(1,n-ii)];
                for kk = ii+1:n
            ...
            % Nisu dobro izracunati r_i kad se koriste expanded bubbles...
            if robot.model == 1
            ...
            d = r(1:n-1) * abs(q(1:n-1)-q_e(1:n-1));
        end
        step = fi/d;
