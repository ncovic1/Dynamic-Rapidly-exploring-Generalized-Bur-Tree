function [graphics_WS, graphics_CS] = DrawObstacles(obstacles, robot, d_theta)
    graphics_WS = {};
    graphics_CS = {};
    ...
    if robot.dim == 2
        ...
        if robot.N_DOF == 2 && nargin == 3
            i = 1;
            subplot(1,2,2); 
            for theta1 = -pi:d_theta:pi
                for theta2 = -pi:d_theta:pi
                    [collision, ~, ~] = CheckCollision(robot, obstacles, [theta1; theta2]);
                    if collision
                        graphics_CS{i} = plot(theta1, theta2,'black','Marker','.','MarkerSize',10); hold on; %drawnow; 
                        i = i + 1;
                    end
                end
            end
