function [collision, distance, separatrix] = DistanceLineSegToCuboid(A, B, obstacle)
    collision = false;
    distance = inf;  
    separatrix = zeros(6,1);
    ...
    if M == 0       % There is no projection of any point
        ...
        [distance, ind] = min(dist_AB_obs);
        S_opt = points(:,ind);
        [~, I] = max(projections(:,ind));
        if I == 1 || I == 4
            D_opt = [obstacle(I); points(2:3,ind)];
        elseif I == 2 || I == 5
            D_opt = [points(1,ind); obstacle(I); points(3,ind)];
        elseif I == 3 || I == 6
            D_opt = [points(1:2,ind); obstacle(I)];
        end
    ...
    function ProjectionLineSegOnSide(min1, min2, min3, max1, max2, max3)
        ...
        for i = 1:2
            if points(min1,i) >= obstacle(min1) && points(min1,i) <= obstacle(max1) && ...
                    points(min2,i) >= obstacle(min2) && points(min2,i) <= obstacle(max2)
                if points(min3,i) >= obstacle(min3) && points(min3,i) <= obstacle(max3)
                    collision = true;
                    return;
                elseif points(min3,i) <= obstacle(min3)
                    projections(min3,i) = true;
                    dist_AB_obs(i) = obstacle(min3) - points(min3,i);
                elseif points(min3,i) >= obstacle(max3)
                    projections(max3,i) = true;
                    dist_AB_obs(i) = points(min3,i) - obstacle(max3);
                end
            end
        end
    end
    ...
    function CheckEdges(tacka, k)
        if projections(1,k)  % Projection on x_min
            CheckCollisionWithMoreSides(1);
            if ~collision
                line_segments = ChooseLineSegments(tacka([2,3]), obstacle(2), obstacle(3), obstacle(5), obstacle(6));
                line_segments = [obstacle(1)*ones(1,size(line_segments,2)); line_segments];
            else
                return;
            end
        elseif projections(4,k)  % Projection on x_max    
            CheckCollisionWithMoreSides(4);
            if ~collision            
                line_segments = ChooseLineSegments(tacka([2,3]), obstacle(2), obstacle(3), obstacle(5), obstacle(6));
        ...
        elseif projections(2,k)  % Projection on y_min
            CheckCollisionWithMoreSides(2);
            if ~collision
                line_segments = ChooseLineSegments(tacka([1,3]), obstacle(1), obstacle(3), obstacle(4), obstacle(6));
        ...
        elseif projections(3,k)  % Projection on z_min
            CheckCollisionWithMoreSides(3);
            if ~collision
                line_segments = ChooseLineSegments(tacka([1,2]), obstacle(1), obstacle(2), obstacle(4), obstacle(5));
                line_segments = [line_segments; obstacle(3)*ones(1,size(line_segments,2))];
            else
                return;
            end
        elseif projections(6,k)  % Projection on z_max 
            CheckCollisionWithMoreSides(6);
            if ~collision               
                line_segments = ChooseLineSegments(tacka([1,2]), obstacle(1), obstacle(2), obstacle(4), obstacle(5));
        ...
        if A(1) < obstacle(1) && B(1) < obstacle(1)
            if A(2) < obstacle(2) && B(2) < obstacle(2)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % < x_min, < y_min, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToPoint(A, B, obstacle([1,2,6]));
                else    % < x_min, < y_min
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([1,2,3]), obstacle([1,2,6]));                    
                end
            elseif A(2) > obstacle(5) && B(2) > obstacle(5)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % < x_min, > y_max, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToPoint(A, B, obstacle([1,5,6]));
                else    % < x_min, > y_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([1,5,3]), obstacle([1,5,6]));                     
                end                    
            else
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % < x_min, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([1,2,6]), obstacle([1,5,6]));
                else    % < x_min
        ...
        elseif A(1) > obstacle(4) && B(1) > obstacle(4)
            if A(2) < obstacle(2) && B(2) < obstacle(2)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % > x_max, < y_min, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToPoint(A, B, obstacle([4,2,6]));                            
                else    % > x_max, < y_min
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([4,2,3]), obstacle([4,2,6]));                    
                end
            elseif A(2) > obstacle(5) && B(2) > obstacle(5)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % > x_max, > y_max, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToPoint(A, B, obstacle([4,5,6]));
                else    % > x_max, > y_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([4,5,3]), obstacle([4,5,6]));                     
                end                    
            else
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % > x_max, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([4,2,6]), obstacle([4,5,6]));
                else    % > x_max
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if A(2) < obstacle(2) && B(2) < obstacle(2)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % < y_min, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([1,2,6]), obstacle([4,2,6])); 
                else    % < y_min
            ...
            elseif A(2) > obstacle(5) && B(2) > obstacle(5)
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % > y_max, > z_max
                    [collision, distance, D_opt, S_opt] = DistanceLineSegToLineSeg(A, B, obstacle([1,5,6]), obstacle([4,5,6]));                             
                else    % > y_max
                ...
                elseif A(3) > obstacle(6) && B(3) > obstacle(6) % > z_max
                    ...
                    CheckCollisionWithMoreSides(1:6);   % Check collision with all sides
                    if ~collision
                        DistanceToMoreLineSegments([obstacle([1,2,3]), obstacle([4,2,3]), obstacle([4,2,3]), obstacle([4,5,3]), ...
                            obstacle([4,5,3]), obstacle([1,5,3]), obstacle([1,5,3]), obstacle([1,2,3]),...
    ...
    function CheckCollisionWithMoreSides(K)
        % K determines which coordinates are constant: [1,2,3,4,5,6] = [x_min, y_min, z_min, x_max, y_max, z_max]
        for k = K
            if k > 3
                kk = k-3;
            else
            ...
            if t >= 0 && t <= 1
                M_t = A([1:kk-1,kk+1:3]) + t*(B([1:kk-1,kk+1:3])-A([1:kk-1,kk+1:3]));
                if M_t >= obstacle([1:kk-1,kk+1:3]) & M_t <= obstacle([4:kk+2,kk+4:6]) % Whether point lies on a side
                    collision = true;
                    distance = 0;
                    return;
                end
